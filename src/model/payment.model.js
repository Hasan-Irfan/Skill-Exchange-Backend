...
const PaymentSchema = new Schema({
  exchange: { type: Schema.Types.ObjectId, ref: "Exchange" }, // Optional - not required for topup/withdrawal
  payer: { type: Schema.Types.ObjectId, ref: "User", required: true },
  payee: { type: Schema.Types.ObjectId, ref: "User" },
  amount: { type: Number, required: true, min: 0 },
  currency: { type: String, default: "USD" },
  type: { type: String, enum: ["deposit","escrow","final_payment","refund","topup","withdrawal","transfer"], default: "escrow" },
  status: { 
    type: String,
    enum: ["initiated","escrowed","captured","refunded","failed","disputed"],
    default: "initiated"
  },
  gateway: { type: String, enum: ["stripe","paypal","manual","wallet"] },
  gatewayRef: String,
  timeline: [{ at: Date, status: String, note: String }]
}, { timestamps: true });

// Indexes for performance
PaymentSchema.index({ exchange: 1, createdAt: -1 }, { sparse: true }); // Sparse since exchange is optional
PaymentSchema.index({ payer: 1, createdAt: -1 });
PaymentSchema.index({ payee: 1, createdAt: -1 });
PaymentSchema.index({ status: 1, createdAt: -1 });
PaymentSchema.index({ type: 1, createdAt: -1 }); // Index for wallet transaction queries
