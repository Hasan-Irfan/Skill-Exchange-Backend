...
export const acceptExchange = async (req, res) => {
    try {
        const exchange = await updateStatusService(req.user, req.params.id, "accept");
...
export const declineExchange = async (req, res) => {
    try {
        const exchange = await updateStatusService(req.user, req.params.id, "decline");
        res.json({ success: true, data: exchange });
    } catch (err) {
        res.status(400).json({ success: false, message: err.message });
    }
...
export const signAgreement = async (req, res) => {
    try {
        const exchange = await signAgreementService(req.user, req.params.id, req.body);
        res.json({ success: true, data: exchange });
    } catch (err) {
        res.status(400).json({ success: false, message: err.message });
    }
...
export const fundEscrow = async (req, res) => {
    try {
        const exchange = await fundEscrowService(req.user, req.params.id, req.body.amount);
        res.json({ success: true, data: exchange });
    } catch (err) {
        res.status(400).json({ success: false, message: err.message });
    }
